...
if ( ! class_exists( 'AS3CF_Compatibility_Check' ) ) {
	...
	class AS3CF_Compatibility_Check {
		...
		public function get_error_msg() {
			...
			$deactivate_link = sprintf( '<a style="text-decoration:none;" href="%s">%s</a>', $deactivate_url, __( 'deactivate', 'as3cf-assets-pull' ) );
			$hide_notice_msg = '<br><em>' . sprintf( __( 'You can %s the %s plugin to get rid of this notice.', 'as3cf-assets-pull' ), $deactivate_link, $this->plugin_name ) . '</em>';
			...
			if ( ! $this->is_parent_plugin_enabled() ) {
				$msg = sprintf( __( '%s has been disabled as it requires the %s plugin.', 'as3cf-assets-pull' ), $this->plugin_name, $parent_plugin_link_html );
				...
				if ( file_exists( WP_PLUGIN_DIR . '/' . $parent_basename ) ) {
					if ( isset( $GLOBALS['aws_meta'][ $this->parent_plugin_slug ]['compatible'] ) && ! $GLOBALS['aws_meta'][ $this->parent_plugin_slug ]['compatible'] ) {
						$msg = rtrim( $msg, '.' ) . ', ' . __( 'which is currently disabled.', 'as3cf-assets-pull' );
					} else {
						$msg          .= ' ' . __( 'It appears to be installed already.', 'as3cf-assets-pull' );
						$activate_url = $this->get_plugin_action_url( 'activate', $parent_basename );
						$msg          .= ' <a id="' . $this->plugin_slug . '-activate-parent" style="font-weight:bold;text-decoration:none;" href="' . $activate_url . '">' . _x( 'Activate it now.', 'Activate plugin', 'as3cf-assets-pull' ) . '</a>';
					}
				} else {
					...
					$msg .= ' ' . sprintf( __( '<a href="%s">Install</a> and activate it.', 'as3cf-assets-pull' ), $install_url );
				}
			...
			if ( ! version_compare( $current_parent_plugin_version, $this->parent_plugin_required_version, '>=' ) ) {
				$msg = sprintf( __( '%s has been disabled as it requires version %s or later of the %s plugin.', 'as3cf-assets-pull' ), $this->plugin_name, $this->parent_plugin_required_version, $parent_plugin_link_html );

				if ( $current_parent_plugin_version ) {
					$msg .= ' ' . sprintf( __( 'You currently have version %s installed.', 'as3cf-assets-pull' ), $current_parent_plugin_version );
				}
				...
				if ( ! empty( $as3cfpro ) && $as3cfpro->get_plugin_slug( true ) === $this->parent_plugin_slug ) {
					...
					if ( ! $as3cfpro->is_valid_licence( false, true ) ) {
						$msg .= ' ' . sprintf( __( 'A valid license for %s is required to update.', 'as3cf-assets-pull' ), $this->get_parent_plugin_name() );
						$msg .= $hide_notice_msg;
				...
				$msg .= ' <a style="font-weight:bold;text-decoration:none;white-space:nowrap;" href="' . $update_url . '">' . __( 'Update to the latest version', 'as3cf-assets-pull' ) . '</a>';
				$msg .= $hide_notice_msg;
			...
			if ( ! isset( $GLOBALS['aws_meta'][ $this->parent_plugin_slug ]['supported_addon_versions'][ $this->plugin_slug ] ) ) {
				$msg = sprintf( __( '%1$s has been disabled because it is not a supported addon of the %2$s plugin.', 'as3cf-assets-pull' ), $this->plugin_name, $this->get_parent_plugin_name() );
			...
			if ( ! version_compare( $this_plugin_version, $this_plugin_version_required, '>=' ) ) {
				$msg = sprintf( __( '%1$s has been disabled because it will not work with the version of the %2$s plugin installed. %1$s %3$s or later is required.', 'as3cf-assets-pull' ), $this->plugin_name, $this->get_parent_plugin_name(), $this_plugin_version_required );

				$update_url  = $this->get_plugin_action_url( 'upgrade', $plugin_basename );
				$upgrade_msg = ' <a style="font-weight:bold;text-decoration:none;white-space:nowrap;" href="' . $update_url . '">' . sprintf( __( 'Update %s to the latest version', 'as3cf-assets-pull' ), $this->plugin_name ) . '</a>';

				global $as3cfpro;
				if ( ! empty( $as3cfpro ) && $as3cfpro->get_plugin_slug( true ) === $this->parent_plugin_slug ) {
					// Don't show update link for addons of a licensed plugin where the license is invalid
					if ( ! $as3cfpro->is_valid_licence( false, true ) ) {
						$upgrade_msg = ' ' . sprintf( __( 'A valid license for %s is required to update.', 'as3cf-assets-pull' ), $this->get_parent_plugin_name() );
					}
				}
		...
		function get_admin_notice() {
			...
			if ( $this->deactivate_if_not_compatible ) {
				$deactivated_msg = sprintf( __( 'The %s plugin has been deactivated.', 'as3cf-assets-pull' ), $this->plugin_name );
		...
		public function get_sdk_requirements_errors() {
			...
			if ( version_compare( PHP_VERSION, '5.5', '<' ) ) {
				$errors[] = __( 'a PHP version less than 5.5', 'as3cf-assets-pull' );
			}

			if ( ! class_exists( 'SimpleXMLElement' ) ) {
				$errors[] = __( 'no SimpleXML PHP module', 'as3cf-assets-pull' );
			}

			if ( ! class_exists( 'XMLWriter' ) ) {
				$errors[] = __( 'no XMLWriter PHP module', 'as3cf-assets-pull' );
			}

			if ( ! function_exists( 'curl_version' ) ) {
				$errors[] = __( 'no PHP cURL library activated', 'as3cf-assets-pull' );

				return $errors;
			}

			if ( ! ( $curl = curl_version() ) || empty( $curl['version'] ) || empty( $curl['features'] ) || version_compare( $curl['version'], '7.16.2', '<' ) ) {
				$errors[] = __( 'a cURL version less than 7.16.2', 'as3cf-assets-pull' );
			}
			...
			if ( ! empty( $curl['features'] ) ) {
				...
				if ( $curl_errors ) {
					$errors[] = __( 'cURL compiled without', 'as3cf-assets-pull' ) . ' ' . implode( ' or ', $curl_errors ); // xss ok
				}
			}

			if ( ! function_exists( 'curl_multi_exec' ) ) {
				$errors[] = __( 'the function curl_multi_exec disabled', 'as3cf-assets-pull' );
			}
		...
		public function get_sdk_error_msg() {
			...
			$msg = __( 'The official Amazon&nbsp;Web&nbsp;Services SDK requires PHP 5.5+ with SimpleXML and XMLWriter modules, and cURL 7.16.2+ compiled with OpenSSL and zlib. Your server currently has', 'as3cf-assets-pull' );
